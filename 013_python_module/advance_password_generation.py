...
class PasswordManager:
    ...
    def _setup_master_password(self):
        ...
        with sqlite3.connect(self.db_name) as conn:
            ...
            cursor.execute(
                "INSERT INTO security_logs (event) VALUES (?)", ("Master password set",)
            )
            conn.commit()
    ...
    def authenticate(self):
        ...
        for attempt in range(1, MAX_ATTEMPTS + 1):
            master_pw = getpass(
                f"Attempt {attempt}/{MAX_ATTEMPTS}: Enter master password: "
            )
            ...
            try:
                ...
                if attempt < MAX_ATTEMPTS:
                    print(
                        f"Invalid password. {MAX_ATTEMPTS - attempt} attempts remaining."
                    )
                else:
    ...
    def _log_security_event(self, event: str):
        ...
        with sqlite3.connect(self.db_name) as conn:
            cursor = conn.cursor()
            cursor.execute("INSERT INTO security_logs (event) VALUES (?)", (event,))
            conn.commit()
    ...
    def generate_password(
        ...
        if not MIN_PW_LENGTH <= length <= MAX_PW_LENGTH:
            raise ValueError(
                f"Password length must be between {MIN_PW_LENGTH} and {MAX_PW_LENGTH}"
            )
    ...
    def _generate_password_flow(self):
        ...
        while True:
            try:
                length = int(
                    input(
                        f"Length ({MIN_PW_LENGTH}-{MAX_PW_LENGTH}, default {DEFAULT_PW_LENGTH}): "
                    )
                    or DEFAULT_PW_LENGTH
                )
                if not MIN_PW_LENGTH <= length <= MAX_PW_LENGTH:
                    print(
                        f"Please enter a value between {MIN_PW_LENGTH} and {MAX_PW_LENGTH}"
                    )
                    continue
                break
            except ValueError:
        ...
        require_each = (
            input("Require at least one of each type? (Y/n): ").lower() != "n"
        )
        ...
        print("\nPassword Analysis:")
        print(
            f"- Strength: {analysis['score']}/4 ({PasswordStrength(analysis['score']).name.replace('_', ' ')})"
        )
        ...
        if store:
            ...
            try:
                self.store_password(
                    service, password, username, notes, auto_generated=True
                )
    ...
    def _export_passwords_flow(self):
        ...
        try:
            exported_file = self.export_passwords(
                format=formats[choice], file_path=file_path if file_path else None
            )
    ...
    def _show_password_stats(self):
        ...
        with sqlite3.connect(self.db_name) as conn:
            ...
            # Length stats
            cursor.execute(
                "SELECT AVG(length), MIN(length), MAX(length) FROM passwords"
            )
        ...
        for score, count in strength_dist:
            print(
                f"- {PasswordStrength(score).name.replace('_', ' ')}: {count} ({count/total:.1%})"
            )
